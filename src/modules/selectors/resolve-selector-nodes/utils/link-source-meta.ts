...
export const linkSourceMeta = (
	...
	const link = (currentNode: parser.Node, depth: number) => {
		const linkedNode = currentNode as ResolvedNode;
		const value = getNormalizedNodeString(currentNode);
		const nodeStart = currentNode.sourceIndex;
		...
		if (linkedNode.type === 'selector') {
			...
			linkedNode.nodes = linkedNode.nodes
				.map((iNode: parser.Node) => link(iNode, depth + 1));
		...
		const matchCandidates = sourceNodeMeta
			.filter(({ depth: sourceDepth, resolvedRange: [metaStart, metaEnd] }) => {
				if (sourceDepth !== depth) return false;
